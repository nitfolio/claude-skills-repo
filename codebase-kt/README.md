...
deliverable distilled from the trail, then asks whether you want it as Markdown, PDF, DOCX, or all three —
PDF/DOCX are generated by invoking Claude's `pdf`/`docx` skills on the Markdown, not hand-rolled.
...
Two more things happen automatically in the background: on `start`, if a `.kt/` trail already exists, it
checks what's changed in the repo since the trail's last recorded checkpoint and flags anything that
might now be stale instead of trusting old findings blindly; and if evidence ever contradicts the initial
repo-type classification, it switches playbooks mid-session rather than forcing a bad fit.
